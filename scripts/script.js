$('document').ready(function () {
    ...
    $('a[href^="#"]').click(function(e){
      e.preventDefault();
        ...
        if (the_id !== '#') {
        //Set a slight delay before scroll to show click effects
          setTimeout(() => {
            $wholePage.animate({
    ...
    $('#nav-icon').click(function(){
      if ($('.main-nav ul').hasClass('display') && $('.main-nav ul').hasClass('slide-in')  ) {
        $(this).removeClass('open');
        $('.main-nav ul').removeClass('slide-in');
        setTimeout(() => {
          $('.main-nav ul').removeClass('display');
        }, 600);
      } else if ( !$('.main-nav ul').hasClass('display') && !$('.main-nav ul').hasClass('slide-in') ) {
        $(this).addClass('open');
        $('.main-nav ul').addClass('display');
        setTimeout(() => {
          $('.main-nav ul').addClass('slide-in');
        }, 30);
      }
    });
    ...
    $(window).scroll(function () {
      const skillsOffset = $('.skills')[0].getBoundingClientRect().top;
      const portfolioOffset = $('#portfolio')[0].getBoundingClientRect().top;
      const aboutOffset = $('main')[0].getBoundingClientRect().top;

      //Show/hide back to top button
      if (aboutOffset < 0 ) {
        $('.arrow-to-top').addClass('display');
      } else {
        $('.arrow-to-top').removeClass('display');
      }

      //Add class if scroll is at Skills section
      if (skillsOffset < 70 && portfolioOffset > 180 ) {
        $('#nav-icon span').css('background', '#e7ff89');
        $('.main-nav ul a').css('color', '#e7ff89');
      } else {
        $('#nav-icon span').removeAttr('style');
        $('.main-nav ul a').removeAttr('style');
      }
    });


    //Code Pen Icon Morphing Animation Using Anime.js
